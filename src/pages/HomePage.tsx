...
export default function HomePage() {
  ...
  async function loadProviders() {
    ...
    mamaFuas?.forEach(p => {
      ...
      if (u) results.push({ user: u, profile: p, packages: pkgMap.get(u.id) || [], type: 'mama_fua' });
    });

    centers?.forEach(p => {
      const u = users.find(x => x.id === p.id);
      if (u) results.push({ user: u, profile: p, packages: pkgMap.get(u.id) || [], type: 'laundry_center' });
    });
